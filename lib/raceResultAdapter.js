...
function buildLapsFromSplits(splits, fallbackRank) {
  ...
  for (const split of splits || []) {
    ...
    const name = cleanString(split.name || split.split || `Круг ${index}`);
    laps.push({
      lapNumber: Number(split.lapNumber) || index,
      name,
      isOnLap: true,
      isLoop: isRaceLoopName(name),
      totalTime,
...
function buildLapsFromWiclaxPassing(raw, fallbackRank) {
  ...
  const checkpoints = buildLapsFromSplits(splits, fallbackRank);
  if (finish) {
    const loops = lapData.length
      ? buildLapsFromLapData(lapData, fallbackRank)
      : buildLapsFromSingle(split || 'Finish', getField(raw, 'time', 'chiptime'), fallbackRank, getField(raw, 'gap')).map(
          (lap) => ({ ...lap, isLoop: true })
        );
    return [...checkpoints, ...loops];
  }
  let laps = checkpoints;
  if (split && !laps.some((lap) => String(lap.name).toLowerCase() === split.toLowerCase())) {
    laps = laps.concat(
      buildLapsFromSingle(split, getField(raw, 'time', 'chiptime'), fallbackRank, getField(raw, 'gap')).map((lap) => ({
        ...lap,
        isLoop: isRaceLoopName(split),
      }))
    );
  }
  return laps;
...
function mapRaceResultAthlete(raw, envelope) {
  ...
  const lastLoop = [...laps].reverse().find((lap) => lap.isLoop !== false);
  const isFinished =
    !isStartList &&
    !dns &&
    (raw.finished === true ||
  ...
  const mapped = {
    ...
    number,
    position,
    resultTime: isFinished
      ? resultTime || lastLoop?.totalTime || lastLap?.totalTime || ''
      : lastLoop?.totalTime || resultTime || lastLap?.totalTime || '',
