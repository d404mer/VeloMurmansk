...
function isRaceResultRow(row) {
  ...
  if (row.dataType === 'passing' || row.dataType === 'result') return true;
  if (row.bib != null || row.realbib != null) return true;
  if (row.firstname != null || row.lastname != null) return true;
  if (row.rank != null && row.name != null && row.time != null) return true;
  if (row.catrank != null && row.name != null) return true;
  if (Array.isArray(row.splits)) return true;
  return false;
}
...
function isRaceResultPayload(body) {
  ...
  if (body.isSuccess === true && Array.isArray(body.data)) return false;
  if (Array.isArray(body.results)) return true;
  if (isRaceResultRow(body)) return true;
  return false;
}
...
function mapRaceResultAthlete(raw) {
  const number = cleanString(raw.bib ?? raw.realbib ?? raw.number ?? raw.nb ?? '');
  const position = toPosition(raw.rank ?? raw.catrank ?? raw.rankbycat ?? raw.position ?? '');
  const resultTime = normalizeTime(raw.time ?? raw.chiptime ?? raw.netto ?? raw.resultTime ?? '');
  const club = cleanString(raw.club ?? raw.team ?? raw.nationality ?? raw.city ?? '');
  const { firstName, lastName } = splitPersonName(raw.name, raw.firstname ?? raw.firstName, raw.lastname ?? raw.lastName);
  const age = cleanString(raw.year ?? raw.age ?? raw.birthyear ?? '');
  const city = cleanString(raw.city ?? raw.team ?? club);

  const splits = Array.isArray(raw.splits) ? raw.splits : [];
  const laps = splits.length
    ? buildLapsFromSplits(splits, position)
    : buildLapsFromSingle(raw.split, raw.time ?? raw.chiptime, position, raw.gap);

  const lastLap = laps.length ? laps[laps.length - 1] : null;
  const status = cleanString(raw.status || raw.finishedstatus || '').toUpperCase();
  const isFinished =
    raw.finished === true ||
    raw.isFinished === true ||
    status === 'FIN' ||
    status === 'FINISHED' ||
    (resultTime !== '' && splits.length > 1 && lastLap && Number(lastLap.lapNumber) > 1 && raw.dataType === 'result');
  const isOnStart = !resultTime && laps.length === 0 && position === '';
  ...
  return {
    ...
    leaderDifference: normalizeGap(raw.gap ?? raw.leaderDifference ?? lastLap?.leaderDifference ?? ''),
    isFinished,
    isOnStart,
    club,
    city,
...
function adaptRaceResult(body, config) {
  ...
  const athletes = fillGapsFromLeader(rows.map(mapRaceResultAthlete));
  return {
    categoryId: inferCategoryId(source, rows, config),
    contestName: cleanString(source.race || rows[0]?.race || ''),
    athletes,
