...
const REGIONS = [
  ...
  { name: 'Ульяновская область', code: 'УЛН', file: 'Ульяновская область.jpg', aliases: ['ульяновская область', 'ульяновск'] },
  {
    ...
    cities: ['Миасс', 'Златоуст', 'Магнитка', 'Магнитогорск'],
  },
...
function regionFlagPathFromClub(club, nationality) {
  const byNationality = regionFlagPath(nationality);
  const byClub = regionFlagPath(club);
  if (looksLikeRegionCode(nationality)) {
    return byNationality || byClub || '';
  }
  return byClub || byNationality || '';
}
