...
const REGIONS = [
  ...
  { name: 'Республика Башкортостан', code: 'БАШ', file: 'Республика Башкортостан.jpg', aliases: ['башкортостан', 'башкирия', 'уфа'] },
  ...
  { name: 'Республика Татарстан', code: 'ТАТ', file: 'Республика Татарстан.jpg', aliases: ['татарстан', 'казань'] },
  ...
  { name: 'Краснодарский край', code: 'КРД', file: 'Краснодарский Край.jpg', aliases: ['краснодарский край', 'краснодар', 'сочи'] },
  ...
  { name: 'Московская область', code: 'МСО', altCodes: ['МОБ'], file: 'Московская область.jpg', aliases: ['московская область', 'подмосковье'] },
  { name: 'Мурманская область', code: 'МУР', file: 'Мурманская область.jpg', aliases: ['мурманская область', 'мурманск'] },
  { name: 'Нижегородская область', code: 'НЖГ', altCodes: ['ННГ'], file: 'Нижегородская область.jpg', aliases: ['нижегородская область', 'нижний новгород', 'н новгород', 'нижний'] },
  ...
  { name: 'Самарская область', code: 'САМ', altCodes: ['СМР'], file: 'Самарская область.jpg', aliases: ['самарская область', 'самара', 'тольятти'] },
  ...
  { name: 'Свердловская область', code: 'СВР', altCodes: ['СВЕ', 'ЕКБ', 'СВД'], file: 'Свердловская область.jpg', aliases: ['свердловская область', 'екатеринбург'] },
  ...
  { name: 'Ульяновская область', code: 'УЛН', file: 'Ульяновская область.jpg', aliases: ['ульяновская область', 'ульяновск'] },
  {
    ...
    code: 'ЧЛБ',
    altCodes: ['ЧЕЛ'],
    file: 'Челябинская область.jpg',
    aliases: ['челябинская область', 'челябинск', 'чел'],
    cities: ['Миасс', 'Златоуст', 'Магнитка', 'Магнитогорск', 'Miass', 'Zlatoust', 'Magnitka', 'Magnitogorsk'],
  },
...
for (const region of REGIONS) {
  ...
  for (const alt of region.altCodes || []) {
    keys.add(normalizeRegionText(alt));
  }
  for (const city of region.cities || []) {
    keys.add(normalizeRegionText(city));
  }
  ...
  for (const city of region.cities || []) {
    ...
    if (key) exactMap.set(key, region);
  }
}

phrases.sort((a, b) => b.alias.length - a.alias.length);

function matchRegionCities(normalized) {
  if (!normalized) return null;
  for (const region of REGIONS) {
    for (const city of region.cities || []) {
      const key = normalizeRegionText(city);
      if (!key || key.length < 4) continue;
      if (normalized === key) return withFlag(region);
      if (normalized.includes(key)) return withFlag(region);
      if (key.includes(normalized) && normalized.length >= 5) return withFlag(region);
    }
  }
  return null;
}
...
function resolveRegion(text) {
  ...
  const byCity = matchRegionCities(normalized);
  if (byCity) return byCity;

  for (const token of normalized.split(' ')) {
    if (!token) continue;
    const tokenHit = exactMap.get(token) || matchRegionCities(token);
...
function looksLikeRegionCode(text) {
  const normalized = normalizeRegionText(text);
  return /^[a-zа-яё]{2,4}$/i.test(normalized);
}
...
function regionFlagPathFromClub(club, nationality, extra) {
  const values = [];
  const push = (value) => {
  ...
  for (const value of values) {
    ...
    if (hit?.flagPath) return hit.flagPath;
  }
  return '';
}
