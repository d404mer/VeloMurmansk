...
function formatLapLabel(completedLap, totalLaps) {
  ...
  if (m > 0) return `КРУГ ${n}/${m}`;
  if (n > 0) return `КРУГ ${n}`;
  return '';
}
...
function createLapTracker() {
  ...
  function simulateLeaderLap(categoryId, fields) {
    ...
    const completedLap = Number(fields.completedLap ?? fields.lapNumber ?? prevCompleted + 1);
    const state = {
      completedLap,
      currentLap: completedLap + 1,
      totalLaps: m,
      ...
      splitTime: fields.splitTime ?? fields.time ?? '',
      updatedAt: new Date().toISOString(),
    };
    lapStates.set(categoryId, state);
