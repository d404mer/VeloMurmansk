...
function createLapTracker() {
  ...
  function collectPlaqueEvents(categoryId, rawAthletes, isFirstPoll, mode = 'leader', splitsFilter = 'loop') {
    const seen = seenPlaques.get(categoryId);
    const plaqueEvents = [];

    for (const raw of rawAthletes || []) {
      for (const lap of visibleLaps(raw, splitsFilter)) {
  ...
  function processRawAthletes(categoryId, rawAthletes, totalLaps, mode = 'leader', splitsFilter = 'loop') {
    ...
    const plaqueEvents = collectPlaqueEvents(categoryId, rawAthletes, isFirstPoll, mode, splitsFilter);
