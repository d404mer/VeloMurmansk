...
function createLapTracker() {
   ...
   * gaps recomputed for everyone who has that split.
   */
  function buildIntermediateBoard(categoryId, rawAthletes, mode = 'leader', completedLap = 0) {
  ...
  function processRawAthletes(categoryId, rawAthletes, totalLaps, mode = 'leader', splitsFilter = 'loop') {
    ensureCategory(categoryId);
    if (totalLaps != null) {
    ...
    const effective = effectiveSplitsFilter(splitsFilter, completedLap);
    ...
    if (effective === 'all' && !isFirstPoll && !willAdvance) {
      intermediateBoard = buildIntermediateBoard(categoryId, rawAthletes, mode, completedLap);
    } else {
      lastIntermediateBoard.set(categoryId, null);
    }
    ...
    return { plaqueEvents, counterUpdated, intermediateBoard, effectiveSplitsFilter: effective };
  }
