...
function resolvePlaqueGap({ totalTime, leaderDifference, place, gap: gapOverride }, mode = 'leader') {
  ...
  if (gapOverride != null && gapOverride !== '') {
    return String(gapOverride);
  }
  if (leaderDifference != null && leaderDifference !== '') {
    return formatGap(leaderDifference);
  }
  return '00:00';
}
...
function createLapTracker() {
  ...
  function collectPlaqueEvents(categoryId, rawAthletes, isFirstPoll, mode = 'leader') {
  ...
  function processRawAthletes(categoryId, rawAthletes, totalLaps, mode = 'leader') {
    ensureCategory(categoryId);
    if (totalLaps != null) {
    ...
    const plaqueEvents = collectPlaqueEvents(categoryId, rawAthletes, isFirstPoll, mode);
