...
function mergeTimingPassings(existingAthletes, passings, config) {
  ...
  for (const raw of existingAthletes || []) {
    ...
    byBib.set(bib, {
      ...raw,
      laps: Array.isArray(raw.laps) ? raw.laps.map((l) => ({ ...l })) : [],
      account: { ...(raw.account || {}) },
    });
  }
  ...
  for (const passing of passings || []) {
    ...
    if (!isStartSplit(passing) && clock) {
      ...
      appendSplitLap(athlete, passing, raceTime, isLoopSplit(passing, config));
      if (isLoopSplit(passing, config)) athlete.isOnStart = false;
    }
    byBib.set(bib, athlete);
  }
...
function mergePassingAthletes(existingAthletes, incomingAthletes) {
  ...
  for (const inc of incomingAthletes || []) {
    ...
    if (!prev) {
      byBib.set(bib, { ...inc, laps: Array.isArray(inc.laps) ? inc.laps.map((l) => ({ ...l })) : [], account: { ...(inc.account || {}) } });
      continue;
    }
    byBib.set(bib, {
      ...prev,
      ...
      laps: inc.laps?.length ? inc.laps.map((l) => ({ ...l })) : prev.laps,
    });
  }
