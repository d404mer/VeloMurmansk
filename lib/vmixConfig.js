...
function formatLapText(lapState) {
  if (!lapState) return '';
  const current =
    Number(lapState.currentLap) ||
    Number(lapState.completedLap) + 1 ||
    Number(lapState.completedLap) ||
    0;
  const total = Number(lapState.totalLaps) || 0;
  if (total > 0) return `КРУГ ${current}/${total}`;
  if (current > 0) return `КРУГ ${current}`;
  return '';
}

function formatLeaderClass(eventName, categoryName, place) {
  const parts = [eventName, categoryName, `${place} Место`].filter(Boolean);
  return parts.join(' • ');
}
