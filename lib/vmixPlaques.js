...
function buildIndexedFields(resolved, config, fieldKeys, includeClass) {
  ...
  for (const key of fieldKeys) {
    fields.push({
      ...
      hint:
        key === 'flag'
          ? 'Путь к флагу региона (авто)'
          : key === 'city'
            ? 'Код региона (авто из клуба)'
            : undefined,
    });
  }
  return fields;
...
function buildLeaderFields(resolved, config, place) {
  ...
  return [
    ...
    },
    {
      ...
      label: 'city',
      vmixConfigKey: 'city',
      vmixFieldName: resolved.singleFields.city || '',
      sourcePath: sourcePathFor(config, 'city'),
      editableSource: true,
      editableVmixName: true,
      vmixStorage: 'single',
      hint: 'Код региона (авто из клуба)',
    },
    {
      key: 'flag',
      label: 'flag',
      vmixConfigKey: 'flag',
      vmixFieldName: resolved.singleFields.flag || '',
      sourcePath: '',
      editableSource: false,
      editableVmixName: true,
      vmixStorage: 'single',
      hint: 'Путь к флагу региона (авто)',
    },
  ];
}

function appendWinnerSlotFields(fields, resolved, config, fieldKeys, dataKey) {
  for (const fieldKey of fieldKeys || []) {
    const isFlag = dataKey === 'flag';
    fields.push({
      key: dataKey,
      label: fieldKey,
      vmixConfigKey: fieldKey,
      vmixFieldName: resolved.singleFields[fieldKey] || '',
      sourcePath: isFlag ? '' : sourcePathFor(config, dataKey),
      editableSource: !isFlag && TARGET_FIELDS.includes(dataKey),
      editableVmixName: true,
      vmixStorage: 'single',
      hint: isFlag ? 'SetImage, поле flag N.Source (авто путь к jpg)' : dataKey === 'city' ? 'Код региона (авто из клуба)' : undefined,
    });
  }
}
...
function buildWinnersFields(resolved, config) {
  ...
  appendWinnerSlotFields(fields, resolved, config, resolved.winnerNameFields, 'name');
  appendWinnerSlotFields(fields, resolved, config, resolved.winnerCityFields, 'city');
  appendWinnerSlotFields(fields, resolved, config, resolved.winnerFlagFields, 'flag');
  return fields;
}
 ...
 * Prefer explicit overrides from the client when provided. Otherwise derive
 * values from plaques, preferring edits that differ from the stored config.
 */
function pickSharedUpdate(previous, candidates) {
  const changed = [];
  for (const value of candidates) {
    if (value !== previous) changed.push(value);
  }
  ...
  let best = changed[changed.length - 1];
  let bestCount = -1;
...
function applyStringMapOverride(target, overrides) {
...
function applyPlaquesToConfig(config, plaques, overrides = {}) {
  ...
  for (const key of TARGET_FIELDS) {
    ...
    const previous = vmix.fieldMapping[key] || '';
    const next = pickSharedUpdate(previous, candidates);
    if (next) vmix.fieldMapping[key] = next;
    else delete vmix.fieldMapping[key];
  }

  // Explicit maps from the web UI win over plaque-derived heuristics.
  applyStringMapOverride(vmix.indexedFields, overrides.indexedFields);
  applyStringMapOverride(vmix.singleFields, overrides.singleFields);
  if (overrides.fieldMapping) {
    for (const key of TARGET_FIELDS) {
      if (!Object.prototype.hasOwnProperty.call(overrides.fieldMapping, key)) continue;
      const next = String(overrides.fieldMapping[key] ?? '').trim();
      if (next) vmix.fieldMapping[key] = next;
      else delete vmix.fieldMapping[key];
    }
  }
