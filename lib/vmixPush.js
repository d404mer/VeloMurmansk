...
function athleteNationality(athlete) {
  const raw = athlete?.raw;
  const value = athlete?.nationality ?? raw?.nationality ?? '';
  return value == null ? '' : String(value).trim();
}

function flagSourceValues(athlete, config) {
  if (!athlete) return [];
  const raw = athlete.raw && typeof athlete.raw === 'object' ? athlete.raw : {};
  return [
    athleteNationality(athlete),
    raw.team,
    raw.club,
    raw.city,
    raw.account?.city,
...
function athleteFieldValue(athlete, fieldKey, config) {
  ...
  if (fieldKey === 'city' || fieldKey === 'flag') {
    ...
    const nationality = athleteNationality(athlete);
    if (fieldKey === 'flag') {
      const explicit = athlete?.flag || athlete?.raw?.flag;
      if (explicit != null && String(explicit).trim() !== '') return String(explicit).trim();
      return regionFlagPathFromClub(club, nationality, flagSourceValues(athlete, config));
    }
    return formatClubDisplay(club, {
      mode: config.clubNameMode,
      nationality,
    });
  }
