...
const { regionCode, regionFlagPath } = require('./regions');

function athleteFieldValue(athlete, fieldKey, config) {
  if (fieldKey === 'city' || fieldKey === 'flag') {
    const club = resolveAthleteValue(athlete, 'city', config.fieldMapping);
    if (fieldKey === 'flag') return regionFlagPath(club);
    return regionCode(club);
  }
...
function appendWinnerFieldGroup(commands, config, tpl, leaders, fieldKeys, dataKey) {
  (fieldKeys || []).forEach((fieldKey, index) => {
    ...
    pushSetText(commands, tpl, selectedName, athleteFieldValue(athlete, dataKey, config));
  });
}
...
function createVmixPusher(getConnection) {
  ...
  function isClientWritable(client) {
    if (!client) return false;
    if (typeof client.connected === 'function' && !client.connected()) return false;
    const socket = client._socket;
    if (!socket) return false;
    if (socket.destroyed || socket.writableEnded || socket.readableEnded) return false;
  ...
  function sendBatch(commands) {
    ...
    if (!isClientWritable(connection.client)) {
      connection.onError?.(new Error('vMix socket is not writable'));
      return;
    }
