...
const SEND_CHUNK_SIZE = 20;   // сколько команд слать за раз
const SEND_CHUNK_DELAY_MS = 30; // пауза между пачками

function createVmixPusher(getConnection) {
  const lastSentValues = new Map(); // `${tpl}|${selectedName}` -> value
  ...
  function sendBatch(commands) {
    ...
    function sendNextChunk() {
      ...
      try {
        ...
        return; // не продолжаем при ошибке соединения
      }
  ...
  function setTextCmd(tpl, selectedName, value) {
    ...
    if (lastSentValues.get(key) === val) return null; // не изменилось
  ...
  function appendSingleField(commands, config, tpl, fieldKey, value) {
    ...
    const cmd = setTextCmd(tpl, selectedName, value);
  ...
  function appendLeaderName(commands, config, tpl, athlete) {
    ...
    if (!selectedName) return;
    const cmd = commands.push(setTextCmd(tpl, selectedName, athleteValue(athlete, 'name')));
    if (cmd) commands.push(cmd);
  }
  ...
  function pushWinnerOverlay(appConfig, command, data, meta = {}) {
    ...
    if (command === 'winner1' && leaders[0]) {
      ...
      const cmd = commands.push(overlayCmd(config.templates.winner1));
      if (cmd) commands.push(cmd);
      ...
      const cmd = commands.push(overlayCmd(config.templates.winner2));
      if (cmd) commands.push(cmd);
      ...
      const cmd = commands.push(overlayCmd(config.templates.winner3));
      if (cmd) commands.push(cmd);
    } else if (command === 'winners') {
      appendWinnersPage(commands, config, leaders, categoryName);
      const cmd = commands.push(overlayCmd(config.templates.winners));
      if (cmd) commands.push(cmd);
