...
const { CURRENT_RACE_CATEGORY_ID, CURRENT_RACE_CATEGORY, ensureCurrentRaceCategory } = require('./currentRace');
...
function applySetup(config, payload) {
  ...
  if (!activeEvent.categories.some((c) => c.id === CURRENT_RACE_CATEGORY_ID)) {
    activeEvent.categories.unshift({
      ...(currentPrev || CURRENT_RACE_CATEGORY),
      id: CURRENT_RACE_CATEGORY_ID,
      name: currentPrev?.name || CURRENT_RACE_CATEGORY.name,
    });
  }
